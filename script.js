...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        const containerRect = container.getBoundingClientRect(); // Fixed typo: getBoundingRect -> getBoundingClientRect
...
function setActiveButton(btn) {
    [mapBtn, graphBtn, documentsBtn].forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
}

// Map button active by default
...
// Placeholder for Documents button (if needed)
documentsBtn.addEventListener('click', () => {
    mapDiv.style.display = 'none';
    graphDiv.style.display = 'none';
    setActiveButton(documentsBtn);
    // Add documents view logic here if required
});
...
function renderGraph() {
    ...
    const mainX = graphWidth / 2;
    const mainSpacing = 400;
    const subSpacing = 60;

    // Type offsets and colors
    const typeOffsets = {
        'newscast': 200,
        'picture': 400,
        'document': 600,
    ...
    const typeColors = {
        'newscast': '#FFA500',     // Orange
        'picture': '#800080',      // Purple
        'document': '#008080',     // Teal
        'article': '#0000FF',      // Blue
        'forum': '#008000',        // Green
        'twitter': '#00BFFF'       // Deep Sky Blue
    };

    // Prepare main nodes from events
    const mainNodes = events.map((event, index) => ({
        ...
        width: 300,
        height: 200 // Fixed height; adjust dynamically if needed
    }));

    // Prepare sub-nodes and links
    const subNodes = [];
    const links = [];
    ...
    events.forEach((event, index) => {
        ...
        event.videoEmbeds.forEach((embed, i) => {
            ...
            links.push({ source: mainId, target: subId });
            if (i > 0) {
                links.push({ source: `video-${index}-${i - 1}`, target: subId });
            }
        });

        if (event.imageUrl) {
            const subId = `picture-${index}`;
            subNodes.push({
                type: 'picture',
                id: subId,
                mainId: mainId,
        ...
        event.validDocuments.forEach((doc, i) => {
            ...
            links.push({ source: mainId, target: subId });
            if (i > 0) {
                links.push({ source: `document-${index}-${i - 1}`, target: subId });
            }
        });

        event.validLinks.forEach((linkObj, i) => {
            const subId = `article-${index}-${i}`;
            subNodes.push({
                type: 'article',
                id: subId,
                mainId: mainId,
            ...
            links.push({ source: mainId, target: subId });
            if (i > 0) {
                links.push({ source: `article-${index}-${i - 1}`, target: subId });
            }
        });

        // Discussion sub-nodes (two per main node, pretend data)
        const discussionTypes = [
            { type: 'forum', text: 'Forum Post' },
            { type: 'twitter', text: 'Twitter Feed' }
        ];
        discussionTypes.forEach((dType, j) => {
            const subId = `${dType.type}-${index}`;
            subNodes.push({
                type: dType.type,
                id: subId,
                mainId: mainId,
    ...
    // Link main nodes vertically
    for (let i = 0; i < mainNodes.length - 1; i++) {
        links.push({ source: mainNodes[i].id, target: mainNodes[i + 1].id });
    }
    ...
    mainNodes.forEach(node => {
        ...
        node.y = currentY;
        currentY += node.height + 200; // Spacing
    });
    ...
    graphDiv.html(''); // Clear previous content
    const svg = graphDiv.append('svg')
        .attr('width', graphWidth)
        .attr('height', currentY + 100);

    // Draw links
    const linkGroup = svg.append('g').attr('class', 'links');
    links.forEach(link => {
        const source = [...mainNodes, ...subNodes].find(n => n.id === link.source);
        const target = [...mainNodes, ...subNodes].find(n => n.id === link.target);
    ...
    mainNodes.forEach(node => {
        ...
        g.append('text')
            ...
            .attr('dy', '0.35em')
            .text(node.text.split('\n')[3]) // Blurb
            .call(wrap, node.width - 20); // Wrap text
    });

    // Sub-nodes
    subNodes.forEach(node => {
        const g = nodeGroup.append('g');
        if (node.type === 'newscast') {
            // Embed video
            ...
            g.append('rect')
                ...
                .attr('height', node.height)
                .attr('fill', 'none')
                .attr('stroke', typeColors[node.type])
                .attr('rx', 10);
            g.append('text')
                .attr('x', node.x + 10)
                .attr('y', node.y + 25)
    ...
    // Text wrapping function (from D3 examples)
