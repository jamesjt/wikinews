...
function setupD3Timeline() {
 ...
 // Add zoom behavior with smoother transition
 const zoom = d3.zoom()
 .scaleExtent([0.1, 10]) // Allow zooming out to 10% and in to 1000%
 .on('zoom', (event) => {
 ...
 // Apply zoom to the SVG with a transition for smoother effect
 svg.call(zoom)
 ...
 // Ensure smooth zooming with transition on wheel or drag
 svg.on('wheel.zoom', (event) => {
 event.preventDefault();
 svg.transition().duration(250).call(zoom.scaleBy, event.deltaY > 0 ? 1 / 1.2 : 1.2);
 });
}
